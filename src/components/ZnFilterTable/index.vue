<template>
  <div class="filter-table">
    ...
    <el-table
      ...
      @current-change="radioHandleSelectionChange"
      @row-click="toDeatils"
    >
      <el-table-column
        :type="hasRadio ? 'index' : 'selection'"
        width="50"
        align="center"
        v-if="multiple"
      >
      ...
      <el-table-column
        ...
        align="left"
        :prop="item.name"
        :min-width="item.label.length <= 6 ? 130 : 200"
        :class-name="item['type'] == 'array' ? 'table_array_class' : ''"
      ...
      >
        <template slot="header">
          ...
          <!-- table表头文字显示-->
          <div slot="reference" class="table-cell-label" v-else>
            <zn-icon :iconName="item.extra.icon" />
            <span class="table-cell-label-text">{{ item.label }}</span>
          </div>
        </template>
        ...
        <template #default="{ row }">
          ...
          <span
            ...
            :style="{
              'min-width': item.extra.labelTagWidth
                ? item.extra.labelTagWidth + 'px'
                : 'unset',
              'text-align': 'center',
            }"
          >
          ...
          <span
            ...
            "
          >
            <span class="column-label-phone">{{ row[item.name] }}</span>
            <span
              class="column-label-call"
              v-permissions="{ permission: ['callBar'] }"
            >
              <zn-icon
                :iconName="'callcenter-phone'"
                @click.stop="makeACall(row[item.name], row)"
              />
            </span>
          </span>
          <!-- 数据是对象{value:"",label:""}  -->
          ...
          <span v-else-if="item['type'] == 'array'" class="table_array">
            <span
              class="table-status-white mr-8"
              v-for="(item, index) of row[item.name]"
              :key="index"
            >
              {{ item }}
            </span>
...
export default {
  ...
  props: {
    ...
    hasHeadPop: {
      type: Boolean, //是否有表头隐藏列 pop 框
      default: () => true,
    },
    hasRadio: {
      type: Boolean, // 是否是单选 默认为多选
      default: () => false,
    },
    tableData: {
      type: Array, // table数据
  ...
  computed: {
    sipType() {
      return this.$store.state.user.sipType
    },
  },
  created() {},
  methods: {
    ...
    is_array(data) {
      return data instanceof Array //true
    },
    // selection
    ...
    // radio
    radioHandleSelectionChange(newVal, oldVal) {
      if (this.deatilsPath) {
    ...
    makeACall(num, card) {
      ...
      if (['2', '3', '4'].includes(statusNumber)) {
        ...
        this.$store.dispatch('callBar/setMemberId', card.member_id)
