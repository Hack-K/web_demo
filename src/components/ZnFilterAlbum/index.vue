<template>
  <div class="filter-album">
    <!-- 相册 -->
    <el-row :gutter="16" v-if="albumData.length > 0">
      <el-col
        :span="24 / rowNumber"
              ...
              >
                ...
                >
                  ...
                  <div
                    ...
                    @click.stop="makeACall(card[item.name], card)"
                  >
                ...
                <div class="info-value pl-8" v-else>
                  ...
                  <span v-else>
                    {{ card[item.name] ? card[item.name].label : '--' }}
                  </span>
                </div>
              </dd>
            </dl>
...
export default {
  ...
  methods: {
    ...
    goDetail(item) {
      if (this.deatilsPath) {
        if (this.deatilsTarget === 'workOrder') {
    ...
    makeACall(num, card) {
      ...
      if (['2', '3', '4'].includes(statusNumber)) {
        ...
        this.$store.dispatch('callBar/setPhoneCallTime', num)
  ...
  computed: {
    ...
    title() {
      return this.finallyColumns.find((item) => item.name === this.indexColumn)
    },
    // 过滤去除 indexColumn 字段
