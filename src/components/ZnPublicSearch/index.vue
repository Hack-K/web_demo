<template>
  <span v-if="theme.showZnSearchBtn">
    <zn-icon :iconName="'search-line'" @click="openDialog" />
      ...
      <el-form :model="queryForm" @submit.native.prevent>
        <el-form-item label-width="0">
          <el-autocomplete
            ...
            @select="handleSelect"
          >
            <template #prefix><zn-icon :iconName="'search-line'" /></template>
          </el-autocomplete>
        </el-form-item>
      </el-form>
    </el-dialog>
...
<script>
  ...
  export default {
    ...
    created() {
      // this.$nextTick(() => {
      //   if (this.theme.showZnSearchBtn) this.loadAll()
      // })
    },
    methods: {
      openDialog() {
