 ...
 * @returns {array}
 */
export function compare(data, type) {
  console.log("data", data)
  let chineseChars = [],
    chars = [],
    list = [];
  data.forEach(item => {
    let name = type === 'club' ? item.member_name : item.nickname
    // 判断是否为中文
  ...
  if(type === 'club'){
    chars.sort((a, b) => a.member_name.charCodeAt(0) - b.member_name.charCodeAt(0));
    chineseChars.sort((a, b) => a.member_name.localeCompare(b.member_name));
  }else{
