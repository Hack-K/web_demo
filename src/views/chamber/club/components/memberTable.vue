<template>
    <div class="member-container height-100" v-loading="exportLoading">
        <zn-query-form>
            <zn-query-form-left-panel>
                <zn-views-tabs :source="'customer'" :options="viewsTabsOptions" :filterOptions="managementOptions"
                    :mark="mark" :views_id="queryForm.views_id" @upTable="getHeader" @allUpdate="getViewsTabs" />
            </zn-query-form-left-panel>
          ...
          /> -->
                <!-- 字段管理 -->
                <zn-field-management-btn ref="ZnFieldManagementBtn" v-if="viewsType == 'table'" :mark="mark"
                    :views_id="queryForm.views_id" :options="managementOptions" @upTable="upTableData" />
                <!-- 卡片管理 -->
                <zn-card-management-btn ref="ZnCardManagementBtn" v-if="viewsType == 'gallery'" :mark="mark"
                    :views_id="queryForm.views_id" :options="managementOptions" @upCard="upCardData" />
                <el-button ref="addButton" type="primary" size="medium" @click="openAdd"
                    v-permissions="{ permission: ['chamber:club:add:member'] }">
        ...
        <div class="content-view table-view" v-else>
            <zn-filter-table v-if="viewsType == 'table'" ref="filterTable" :class="total === 0 ? 'table-view-empty' : ''"
                :multiple="true" :tableData="tableList" :finallyColumns="finallyColumns" :deatilsPath="deatilsPath"
                :maxHeight="'650'" @selectList="getSelect" @fetch-data="fetchData"
                :defaultPageImgUrl="require('@/assets/images/default_page/pic-currency.png')" :defaultPageTips="'暂无数据'">
                <el-button type="text" size="medium" class="delete-btn" icon="el-icon-delete" @click="delMember">
                    移除成员
                </el-button>
            </zn-filter-table>
            <zn-pagination v-show="total > 0" :page.sync="queryForm.page" :limit.sync="queryForm.listRows"
                @pagination="fetchData" :total="total" :algin="'right'" />
        </div>
        <!-- 添加成员抽屉 -->
        <add-member v-if="memberDrawer" ref="addMember" :title="title" @fetch-data="fetchData" />
    </div>
</template>
  
<script>
import { getMemberList, delClubMember } from "@/api/chamber/club"
...
export default {
    ...
    data() {
        return {
            memberDrawer: false,  // 添加成员抽屉
            mark: "CmGroupMember", //特定标识,根据业务模块不同,传输的标识也不同,标识由后端定义(或者字典维护)
    ...
    methods: {
        ...
        openAdd() {
            ...
            this.$nextTick(() => {
                this.$refs["addMember"].show(this.id)
            })
        },
        // 删除成员
        delMember() {
            this.$confirm("确认移除专委会成员？", "警告", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning",
            }).then(res => {
                delClubMember({
                    id_list: this.moreList.map(item => item.id)
                }).then(async res => {
                    if (res.code === 200) {
                        this.$notify({
                            title: "移除成功",
