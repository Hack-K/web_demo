<template>
  <div class="activity-container parent-container" v-loading="exportLoading">
    <zn-query-form>
      ...
      <zn-query-form-right-panel>
        ...
        <el-button
          ...
          size="medium"
          @click="openAdd"
          v-permissions="{ permission: ['chamber:article:add'] }"
        >
    ...
    <div class="content-view table-view" v-else>
      ...
      >
        <el-button
          ...
          v-permissions="{ permission: ['chamber:article:del'] }"
          size="medium"
          class="delete-btn"
          icon="el-icon-delete"
          @click="delArticle"
        ...
        <el-button
          v-if="
            moreList.length === 1 &&
            tableItemSelect['publish_status']['value'] == 1
          "
          type="text"
          size="medium"
          class="delete-btn"
          icon="el-icon-delete"
          @click="articleStatus(0)"
        >
          取消发布
        </el-button>
        <el-button
          v-if="
            moreList.length === 1 &&
            tableItemSelect['publish_status']['value'] == 0
          "
          @click="articleStatus(1)"
          type="text"
          style="color: #005bd9"
          v-permissions="{ permission: ['chamber:article:publish'] }"
        >
          <zn-icon :iconName="'artcle-publish'" />
          <span>发布</span>
        </el-button>
      </zn-filter-table>
      <zn-pagination
    ...
    <addArtcle
      v-if="addArtcleDrawer"
      ref="addArtcle"
      :drawer="addArtcleDrawer"
      @closeDrawer="closeDrawer"
    />
    <OpenPreviewImage v-if="false" />
    <OpenPreviewArticle v-if="false" />
  </div>
</template>
...
import addArtcle from './components/addArtcle'
import OpenPreviewImage from './components/OpenPreviewImage.vue'
import OpenPreviewArticle from './components/OpenPreviewArticle.vue'
export default {
  name: 'allCustomer',
  components: {
    addArtcle,
  ...
  data() {
    return {
      addArtcleDrawer: false, //添加文章抽屉
      mark: 'Article', //特定标识,根据业务模块不同,传输的标识也不同,标识由后端定义(或者字典维护)
  ...
  methods: {
    ...
    // 打开新建抽屉
    openAdd() {
      this.addArtcleDrawer = true
      // this.$refs['addArtcle'].open()
    },
    // 关闭新建抽屉
    closeDrawer() {
      this.addArtcleDrawer = false
      this.fetchData()
    },
    rowDetailClick(e) {
      detailArticle({ id: e['id'] }).then((res) => {
        let { data } = res
        this.openAdd()
        this.$nextTick(() => {
          this.$refs['addArtcle'].open(data)
        })
        // this.$refs['addArtcle'].open(data)
      })
    },
    delArticle() {
      let id_list = this.moreList.map((item) => {
        return item['id']
      })
      this.$confirm('确认删除该文章', '警告', {
        ...
        .then(async (_) => {
          await deleteArticle({ id_list })
          await this.fetchData()
        })
        .catch()
    },
    articleStatus(publish_status) {
      let id_list = [this.tableItemSelect['id']]
      updateStatusArticle({ id_list, publish_status }).then(async (res) => {
        await this.fetchData()
      })
    },
  },
