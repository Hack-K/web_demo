<template>
  <div class="role-container">
    <el-row :gutter="16" class="role-row">
      <el-col :span="6" class="role-col-left">
        <el-card class="card-wrap">
          ...
          <div class="role-tree-views" v-if="roleList.length > 0">
            <el-col
            ...
            >
              ...
              >
                <zn-icon :iconName="'icon-user'" class="mr-10" />
                <span>{{ item.name }}</span>
              </el-col>
              ...
              <el-col :span="2" class="more-pop">
                ...
                >
                  <div
                    class="pop-li"
                    v-permissions="{
                      permission: ['system:roleManagement:edit'],
                    }"
                  >
                    <el-popover
                      width="348"
                    ...
                    >
                      ...
                      <el-button
                        type="text"
                        size="medium"
                        @click="someoneForm.name = item.name"
                        slot="reference"
                        v-permissions="{
                          permission: ['system:roleManagement:edit'],
                        }"
                  ...
                  <div
                    class="pop-li"
                    v-permissions="{
                      permission: ['system:roleManagement:del'],
                    }"
                  >
                    <el-button
                      type="text"
          ...
          <el-col :span="24" class="role-footer">
            <el-button
              type="primary"
              @click="openRoleAdd"
              v-permissions="{ permission: ['system:roleManagement:add'] }"
            >
      ...
      <el-col :span="18" class="role-col-right">
        <el-button
          ...
          @click="switchEdit"
          v-if="currentTab == 'roleInformation' && !roleInformationEdit"
          v-permissions="{ permission: ['system:roleManagement:edit'] }"
        >
        ...
        >
          <!-- 角色信息 -->
          <el-tab-pane label="角色信息" name="roleInformation">
            <!-- 角色信息详情 -->
            <role-information-details
              ref="RoleInformationDetails"
              :switchEdit="roleInformationEdit"
              :deatils="deatilsData"
              :treeData="treeData"
              :referenceData="referenceData"
              @fetch-data="fetchRoleDeatils"
              ...
              <zn-query-form class="ml-16 mr-16">
                ...
                <zn-query-form-right-panel class="form-right">
                  ...
                  <el-button
                    type="primary"
                    size="medium"
                    @click="openAssociatedMember"
                    v-permissions="{
                      permission: ['system:roleManagement:relation'],
                    }"
                  >
              ...
              <div class="content-view table-view" v-else>
                ...
                >
                  <el-button
                    ...
                    @click="removeMembers"
                    v-permissions="{
                      permission: ['system:roleManagement:remove'],
                    }"
                  >
    ...
    <edit
      ref="edit"
      @fetch-data="getRoleList"
      :referenceData="referenceData"
      :modalToBody="false"
    />
    <!-- 关联成员 -->
    <associated-member
      ref="AssociatedMember"
      @fetch-data="fetchData"
      :modalToBody="false"
    />
  </div>
...
import AssociatedMember from './components/AssociatedMember'
import RoleInformationDetails from './components/RoleInformationDetails'
import {
  getList,
  details,
  roleEditName,
  ...
  userDel,
  tplList,
} from '@/api/system/roleManagement'
...
export default {
  ...
  data() {
    return {
      mark: 'RoleUser', //特定标识,根据业务模块不同,传输的标识也不同,标识由后端定义(或者字典维护)
      isActive: 0, //当前选中角色
      hoverIndex: '', //鼠标hover角色
      roleList: [], //角色列
      deatilsData: {}, //角色信息
      someoneForm: {
        //修改角色Form
  ...
  methods: {
      ...
      getList().then((res) => {
        if (res.code == 200) {
          ...
          if (this.roleList.length) {
            this.fetchRoleDeatils(this.roleList[0].id)
            this.queryForm.admin_role_id = this.roleList[0].id //初始值为左侧列表第一个角色的id
          }
        }
      })
    },
    ...
    getAuthTree() {
      authTree().then((res) => {
        ...
        this.$refs.RoleInformationDetails.$refs.tree.setCheckedKeys(
    ...
    handlerNode(id, index) {
      this.isActive = index
      this.queryForm.admin_role_id = id
      this.fetchRoleDeatils(id) //刷新详情
      this.fetchData() //刷新关联成员列表数据(仅列表)
    },
    // 打开新增抽屉
    openRoleAdd() {
      this.$refs.edit.showEdit(this.referenceData)
    },
    // 取消左侧编辑角色名称
    cancelEditLeft(index) {
      this.$refs["roleEditPopover"][index].doClose()
      this.someoneForm.name = ''
    },
    // 编辑操作pop框显示监听
    editPopoverShow(){
      if(this.$refs["roleTreePopover"]){
        this.$refs["roleTreePopover"].forEach(item => {
          item.doClose()
        })
      }
    },
    ...
    handlerEdit(id, index) {
      roleEditName({ id: id, name: this.someoneForm.name }).then((res) => {
        if (res.code == 200) {
          this.$notify({
            title: '成功',
            message: '修改成功！',
    ...
    handlerDel(item,index) {
      if(this.$refs["roleTreePopover"]){
        this.$refs["roleTreePopover"][index].doClose()
      }
      this.$confirm('确认删除' + `${item.name}` + '角色？', '警告', {
    ...
    // 右侧
    setSwitch(val) {
      this.roleInformationEdit = val
    },
    // tabs切换
    tabClick(tab, event) {
      this.currentTab = tab.name
      if (!this.currentTab == 'roleInformation') {
        this.roleInformationEdit = true
      }
    ...
    // 打开关联成员抽屉
    openAssociatedMember() {
      this.$refs['AssociatedMember'].show(this.queryForm.admin_role_id)
    },
    // 移除成员
    removeMembers() {
      this.$confirm('是否确认移除？', '警告', {
        ...
        userDel({
          ids: this.moreList.map((v) => v.id),
          role_id: this.queryForm.admin_role_id,
        }).then((res) => {
          if (res.code == 200) {
            this.$notify({
              title: '成功',
